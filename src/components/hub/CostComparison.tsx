...
function Row({ label, value, muted = false }: { label: string; value: string; muted?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-3 py-1 text-sm">
      <span className={cn("truncate", muted ? "text-muted-foreground" : "text-foreground/80")}>{label}</span>
...
export function CostComparison({
  ...
  return (
    <section className={cn("overflow-hidden rounded-xl border bg-card", className)} aria-label="Cost comparison">
      <header className="flex flex-wrap items-center justify-between gap-2 border-b bg-muted/40 px-4 py-3">
        <div>
          <h3 className="text-sm font-semibold text-foreground">Cost comparison · {treatmentLabel}</h3>
          <p className="text-xs text-muted-foreground">Singapore benchmark versus complete Batam package</p>
        </div>
      ...
      <div className="grid gap-px bg-border sm:grid-cols-2">
        <div className="bg-card p-4">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-semibold uppercase tracking-wide text-singapore">Singapore</h4>
          </div>
          ...
          <div className="mt-3 flex items-baseline justify-between border-t pt-3">
            ...
            <span className="text-xl font-semibold tabular-nums text-singapore">{sgd(totals.benchmarkTotal)}</span>
          </div>
        </div>
        ...
        <div className="bg-card p-4">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-semibold uppercase tracking-wide text-batam">Batam medical tourism</h4>
          </div>
          <div className="mt-3 divide-y divide-border/60">
            {medicalKeys
          ...
          <div className="mt-3 flex items-baseline justify-between border-t pt-3">
            <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Total</span>
            <span className="text-xl font-semibold tabular-nums text-batam">{sgd(totals.packageTotal)}</span>
          </div>
        </div>
      </div>
      ...
      <footer className="journey-gradient flex flex-wrap items-center justify-between gap-3 px-4 py-3 text-primary-foreground">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[11px] font-medium uppercase tracking-wide opacity-90">Estimated saving</p>
            <p className="text-lg font-semibold tabular-nums">
              {sgd(totals.savings)} <span className="text-sm font-medium opacity-90">· {pct(totals.savingsPct)}</span>
            </p>
          </div>
        </div>
      </footer>
